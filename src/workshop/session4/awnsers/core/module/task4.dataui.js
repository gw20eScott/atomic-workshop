...
const TextComponent = ({ name, key, data, validation, handleChange }) => {
    ...
    return <div className='fieldEntry'>
        <label>
            {name}
        </label>
        <input value={data[key]} onChange={e => {
            passValidation(e.target.value)
        }} />
        {error !== false && <div className='errorText'>
            {error}    
        </div>}
    </div>
}
...
const TextAreaComponent = ({ name, key, data, validation, handleChange }) => {
    ...
    return <div className='fieldEntry'>
        ...
        <textarea value={data[key]} onChange={e => {
            passValidation(e.target.value)
        }} />
        {error !== false && <div className='errorText'>
            {error}    
...
const SelectComponent = ({ name, options, data, key, handleChange }) => {
    return <div className='fieldEntry'>
        <label>{name}</label>
        <select value={data[key]} name={name} id={name} onChange={(e) => {
            handleChange(key, e.target.value)
        }}>
            {
                options.map(({ name, value }) => <option key={name} value={value}> {value} </option>)
            }
        </select>
    </div>
}
