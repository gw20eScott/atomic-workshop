...
const ProductModule = ({ title, showToggles = false, description, activeProducts, data, notForSale, handleClick }) => {
    const [variant, setVariant] = useState('grid')
    
    return <ProductModuleWrapper variant={variant}>
        {title && <PageTitle>
            {title}
        </PageTitle>}
        {description && <P>
        ...
        {showToggles && <div className='toggleView'>
            <div onClick={() => setVariant('grid')}>
                Grid
            </div>
            <div onClick={() => setVariant('list')}>
                List
            </div>
        </div>}
        ...
        <div className='productList'>
            {
                data && data.map(item => <ProductCard
                    variant={variant}
